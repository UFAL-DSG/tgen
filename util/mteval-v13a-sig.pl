...
die "Error in command line:  stats_file not defined$usage" unless ( $stat_file );
my $BLEU_BP;
...
# extra code inserted by Kevin Gimpel for printing segment-level stats for significance testing:
       ...
       for (my $j=1; $j<= $max_Ngram; $j++) {
               print STATS_FILE " $match_cnt->[$j]";
       }
       print STATS_FILE " |";
       for (my $j=1; $j<= $max_Ngram; $j++) {
               print STATS_FILE " $tst_cnt->[$j]";
       }
       print STATS_FILE " |";
       for (my $j=1; $j<= $max_Ngram; $j++) {
               print STATS_FILE " $ref_cnt->[$j]";
       }
       print STATS_FILE " |";
       for (my $j=1; $j<= $max_Ngram; $j++) {
               print STATS_FILE " $tst_info->[$j]";
       }
       print STATS_FILE " |";
       for (my $j=1; $j<= $max_Ngram; $j++) {
               print STATS_FILE " $ref_info->[$j]";
       }
       print STATS_FILE " | $sys\n";
               # $match_count[$j] = $tst_count[$j] = $ref_count[$j] = $tst_info[$j] = $ref_info[$j] = 0;
               # print STATS_FILE "$tst_cnt $ref_cnt $tst_info $ref_info $sys %DOCmt\n";
# end inserted code
